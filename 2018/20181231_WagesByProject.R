...
codes <- read.table("projectCodes_2018.csv", header = F, fill = T, skip = 1, sep = ",")
...
last.names <- c("Cronan", "Restaino", "Eagle", "Yang", "Wiggins", "Nemens")
...
incorrect.names <- c("Restanio", "Paige", "Holiday", "Ernesto")
...
#rd_remove_bad_names <- raw.data[!(lastName.List == "DELETE") == T,]
rd_remove_bad_names <- raw.data
...
col2.2 <- as.character(rd_remove_bad_names$V2)
...
{ 
  ...
  {
    if(is_empty(unlist(str_extract_all(col2.2[v], project.codes[i]))) == T)
    {projectCode.List[v] <- projectCode.List[v]} else
    {projectCode.List[v] <- unlist(str_extract_all(col2.2[v], project.codes[i]))}
  }
}
